...
function Chat() {
  ...
  const systemPromptLoaded = useRef(false);
  ...
  const agent = useAgent<ChatAgent>({
    ...
    onStateUpdate: useCallback((state: ChatAgent) => {
      ...
      setGuardrails(s?.guardrailsEnabled ?? true);
      if (s?.model) setModel(s.model);
      if (!systemPromptLoaded.current && typeof s?.systemPrompt === "string") {
        setSystemPrompt(s.systemPrompt);
        systemPromptLoaded.current = true;
      }
    }, []),
    onError: useCallback(
  ...
  const toggleGuardrails = async (next: boolean) => {
    setGuardrails(next);
    try {
      await agent.stub.setGuardrails(next);
    } catch (e) {
      console.error("Failed to toggle guardrails:", e);
    }
  };
  ...
  const changeModel = async (next: string) => {
    setModel(next);
    try {
      await agent.stub.setModel(next);
    } catch (e) {
  ...
  const saveSystemPrompt = async () => {
    try {
      await agent.stub.setSystemPrompt(systemPrompt);
    } catch (e) {
      console.error("Failed to set system message:", e);
    }
  };
  ...
  return (
    <div className="flex h-screen bg-kumo-elevated">
      ...
      >
        ...
        {/* Header */}
        <header
          className="px-5 py-4 bg-kumo-base border-b border-kumo-line"
          ...
          <div className="max-w-3xl mx-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                aria-label="Open branding admin"
                onClick={() => {
            ...
            <div className="flex items-center justify-between gap-3 mb-2 flex-wrap">
              <div className="flex items-center gap-1.5">
                <ShieldCheckIcon
                  size={14}
                  className={
                    guardrails ? "text-kumo-success" : "text-kumo-inactive"
                  }
                />
                <Text size="xs" variant="secondary">
                  Guardrails
                ...
                <Switch
                  ...
                  size="sm"
                  aria-label="Toggle AI Gateway guardrails"
                />
              </div>
              <select
                value={model}
                onChange={(e) => changeModel(e.target.value)}
                aria-label="Select Workers AI model"
                className="max-w-[260px] px-2 py-1.5 text-xs rounded-lg border border-kumo-line bg-kumo-base text-kumo-default focus:outline-none focus:ring-1 focus:ring-kumo-accent"
              >
                {modelOptions.map((m) => (
      ...
      {showPanel && (
        <aside className="w-80 shrink-0 border-l border-kumo-line bg-kumo-base flex flex-col overflow-y-auto">
          ...
          <div className="p-4 space-y-2">
            ...
            <Text size="xs" variant="secondary">
              Appended to the system prompt sent with every message.
            </Text>
            <textarea
              value={systemPrompt}
              onChange={(e) => setSystemPrompt(e.target.value)}
              onBlur={saveSystemPrompt}
              placeholder="e.g. You are a helpful support agent for Acme Corp. Always answer concisely."
              rows={12}
            ...
            />
            <Text size="xs" variant="secondary">
              Changes save when you click away from the field.
            </Text>
          </div>
        </aside>
      )}
